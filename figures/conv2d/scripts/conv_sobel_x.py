...
def apply_sobel_x_convolution(image_path, output_path):
    ...
    kernel_table = Table(title="Sobel X Kernel Configuration")
    ...
    console.print(Panel(f"[bold green]Sobel X convolution completed successfully![/bold green]\n"
                       f"Result saved to: [yellow]{output_path}[/yellow]", 
                       title="Processing Complete"))

def main():
    """
    Main function to run the Sobel X convolution script.
    """
    parser = argparse.ArgumentParser(description='Sobel X Convolution Example')
    ...
    console.print(Panel("[bold blue]Sobel X Convolution Script[/bold blue]\n"
                       "Vertical edge detection using Sobel X kernel", 
    ...
    # Set output path
    output_path = os.path.join(args.output_dir, "conv_sobel_x.png")
