...
def apply_roberts_y_convolution(image_path, output_path):
    ...
    kernel_table = Table(title="Roberts Y Kernel Configuration")
    ...
    console.print(Panel(f"[bold green]Roberts Y convolution completed successfully![/bold green]\n"
                       f"Result saved to: [yellow]{output_path}[/yellow]", 
                       title="Processing Complete"))

def main():
    """
    Main function to run the Roberts Y convolution script.
    """
    parser = argparse.ArgumentParser(description='Roberts Y Convolution Example')
    ...
    console.print(Panel("[bold blue]Roberts Y Convolution Script[/bold blue]\n"
                       "Diagonal edge detection using Roberts Y kernel", 
    ...
    # Set output path
    output_path = os.path.join(args.output_dir, "conv_roberts_y.png")
