...
def apply_prewitt_y_convolution(image_path, output_path):
    ...
    kernel_table = Table(title="Prewitt Y Kernel Configuration")
    ...
    console.print(Panel(f"[bold green]Prewitt Y convolution completed successfully![/bold green]\n"
                       f"Result saved to: [yellow]{output_path}[/yellow]", 
                       title="Processing Complete"))

def main():
    """
    Main function to run the Prewitt Y convolution script.
    """
    parser = argparse.ArgumentParser(description='Prewitt Y Convolution Example')
    ...
    console.print(Panel("[bold blue]Prewitt Y Convolution Script[/bold blue]\n"
                       "Horizontal edge detection using Prewitt Y kernel", 
    ...
    # Set output path
    output_path = os.path.join(args.output_dir, "conv_prewitt_y.png")
