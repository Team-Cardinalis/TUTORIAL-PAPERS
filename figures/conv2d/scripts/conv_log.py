#!/usr/bin/env python3
"""
LoG (Laplacian of Gaussian) Convolution Example
Generates edge detection using LoG kernel on sample.png

This script demonstrates edge detection using the LoG kernel:
[[ 0, -1,  0],
 [-1,  4, -1],
 [ 0, -1,  0]]
...
def apply_log_convolution(image_path, output_path):
    """
    Apply LoG (Laplacian of Gaussian) convolution to detect edges.
    ...
    # Define LoG kernel for edge detection
    log_kernel = np.array([
        [ 0, -1,  0],
        [-1,  4, -1],
        [ 0, -1,  0]
    ...
    kernel_table = Table(title="LoG Kernel Configuration")
    ...
    kernel_table.add_row("Purpose", "Edge Detection")
    kernel_table.add_row("Method", "Laplacian of Gaussian")
    ...
    with console.status("[bold magenta]Creating visualization...", spinner="dots"):
        ...
        im = axes[1].imshow(log_kernel, cmap='RdBu_r', vmin=-1, vmax=4)
    ...
    console.print(Panel(f"[bold green]LoG convolution completed successfully![/bold green]\n"
                       f"Result saved to: [yellow]{output_path}[/yellow]", 
                       title="Processing Complete"))

def main():
    """
    Main function to run the LoG convolution script.
    """
    parser = argparse.ArgumentParser(description='LoG Convolution Example')
    ...
    console.print(Panel("[bold blue]LoG Convolution Script[/bold blue]\n"
                       "Edge detection using Laplacian of Gaussian kernel", 
    ...
    # Set output path
    output_path = os.path.join(args.output_dir, "conv_log.png")
