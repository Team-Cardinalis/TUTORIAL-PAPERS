...
def apply_prewitt_x_convolution(image_path, output_path):
    ...
    kernel_table = Table(title="Prewitt X Kernel Configuration")
    ...
    kernel_table.add_row("Purpose", "Vertical Edge Detection")
    kernel_table.add_row("Direction", "Horizontal Gradient")
    ...
    console.print(Panel(f"[bold green]Prewitt X convolution completed successfully![/bold green]\n"
                       f"Result saved to: [yellow]{output_path}[/yellow]", 
                       title="Processing Complete"))

def main():
    """
    Main function to run the Prewitt X convolution script.
    """
    parser = argparse.ArgumentParser(description='Prewitt X Convolution Example')
    ...
    console.print(Panel("[bold blue]Prewitt X Convolution Script[/bold blue]\n"
                       "Vertical edge detection using Prewitt X kernel", 
    ...
    # Set output path
    output_path = os.path.join(args.output_dir, "conv_prewitt_x.png")
